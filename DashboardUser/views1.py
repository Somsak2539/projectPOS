...
# ✅ ใช้ `defaultdict(float)` เพื่อเก็บค่ารวมโดยอัตโนมัติ
    daily_profit = defaultdict(float)
    weekly_profit = defaultdict(float)
    monthly_profit = defaultdict(float)

    for record in records1:
    # ✅ แปลง timestamp ให้เป็น timezone ไทย
        timestamp_th = record.timestamp.astimezone(bangkok_tz)

    # ✅ แปลง timestamp เป็น Date / Week / Month ตาม timezone ไทย
        date = timestamp_th.date()  
    ...
    # ✅ รวม totalProfit เป็น sum ต่อวัน, สัปดาห์, เดือน
        for item in stock_adjustments:
            if isinstance(item, dict):  
                total_profit = float(item.get("totalProfit", 0))

            # ✅ DEBUG แสดงค่าที่ถูกต้อง
                print("📅 [DEBUG] เวลาของไทย:", timestamp_th.strftime("%Y-%m-%d %H:%M:%S %Z"))
                print("💰 [DEBUG] ค่า totalProfit:", total_profit)

            # ✅ รวมค่าใน `dict`
                daily_profit[date] += total_profit
                weekly_profit[week] += total_profit
                monthly_profit[month] += total_profit
