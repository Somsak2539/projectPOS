...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // ✅ ฟังก์ชันดึงข้อมูลจาก Django API ตามช่วงเวลาที่เลือก
    function fetchSalesData(filterType = "year") {
        console.log(`🟢 เรียก API: /indexDashboardUser/?format=json&filter=${filterType}`);
        
        fetch(`/indexDashboardUser/?format=json&filter=${filterType}`)
            .then(response => response.json())
            .then(data => {
                console.log("✅ ข้อมูลที่ได้รับจาก API:", data);
                // ✅ อัปเดตช่วงวันที่ที่แสดง
                // ✅ ตรวจสอบว่ามี start_date และ end_date หรือไม่
                if (data.start_date && data.end_date) {
                    updateDateRange(data.start_date, data.end_date);
                } else {
                    console.error("❌ start_date หรือ end_date ไม่ถูกต้อง", data);
                }


                // ✅ ตรวจสอบข้อมูลสินค้ายอดขาย
                if (!data.product_sales || data.product_sales.length === 0) {
                    console.error("❌ ไม่มีข้อมูลสินค้า");
                    return;
                }

                // ✅ ดึงข้อมูลสินค้าและยอดขาย
                let productNames = data.product_sales.map(item => item.x);  // ชื่อสินค้า
                let salesData = data.product_sales.map(item => parseFloat(item.y)); // จำนวน kg ที่ขายได้

                // ✅ อัปเดตกาแฟแท่ง ApexCharts
                updateChart(productNames, salesData);
            })
    ...
    function updateDateRange(startDate, endDate) {
        document.querySelector("#selectedDateRange").textContent = 
            `ช่วงวันที่: ${startDate} ถึง ${endDate}`;


    }
    // ✅ ฟังก์ชันอัปเดตกาแฟแท่ง ApexCharts
    function updateChart(productNames, salesData) {
        var options = {
            series: [{
                name: "ยอดขาย (ต่อหน่วย)",
                data: salesData
            }],
            chart: {
                type: 'bar',
            ...
            colors: ["#FF5733"], // ✅ ใช้สีแดง
            dataLabels: {
                enabled: true,
                style: {
                    fontSize: '14px',
                    colors: ["#000000"] // ✅ เปลี่ยนตัวเลขเป็นสีดำ
                },
                formatter: function (val) { // ✅ แสดงตัวเลขเป็นทศนิยม 2 ตำแหน่ง
                    return val.toFixed(2);
                }
            },tooltip: {
                y: {
                    formatter: function(value) {
                        return value.toFixed(2); // ✅ ทำให้ tooltip แสดงเป็นทศนิยม 2 ตำแหน่ง
                    }
                }
            },
            xaxis: {
        ...
        // ✅ ล้างกราฟเก่าก่อนอัปเดตใหม่
        document.querySelector("#basicBar").innerHTML = "";
        var chart = new ApexCharts(document.querySelector("#basicBar"), options);
        chart.render();
    }

    // ✅ เพิ่ม Event ให้ปุ่มสำหรับเลือกช่วงเวลา
    document.querySelector("#filterDaily1").addEventListener("click", () => fetchSalesData("day"));
    document.querySelector("#filterWeekly1").addEventListener("click", () => fetchSalesData("week"));
    document.querySelector("#filterMonthly1").addEventListener("click", () => fetchSalesData("month"));
    document.querySelector("#filterYearly1").addEventListener("click", () => fetchSalesData("year"));

    // ✅ โหลดค่าเริ่มต้นเป็นรายปี
    fetchSalesData("day");
});
