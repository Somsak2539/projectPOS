...
function addRemoveEvent() {
 ...
 /*document.addEventListener("DOMContentLoaded", function () {
    ...
    // ✅ ดึงข้อมูลสินค้าโดยใช้ AJAX
    fetch("/get-product-list/")  // 🔹 ต้องเปลี่ยนเป็น URL ที่ใช้ดึงสินค้า
        .then(response => response.text())
        .then(html => {
            document.getElementById("product-container").innerHTML = html;
            console.log("✅ โหลด product_list_ajax.html สำเร็จ!");
        })
        .catch(error => console.error("❌ ไม่สามารถโหลด product_list_ajax.html", error));
});*/
